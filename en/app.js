(() => {
  ...
  const researchLabels = {
    ...
    questionPosition: (current, total) => `Question ${current} of ${total}. All are reviewed by people.`
  };
  ...
  if (topic) {
    const defaultOption = topic.dataset.defaultOption || "All topics";
  ...
  const getVisibleRecords = () => {
    ...
    const records = catalog.filter((record) => {
      ...
      const searchText = [record.title, record.provider, record.category, record.coverage, record.description, record.temporalCoverage, record.spatialResolution, record.license, ...record.formats, ...tagTerms]
        .join(" ")
        .toLocaleLowerCase();
  ...
  const injectStructuredData = () => {
    const structuredData = {
      ...
      "dataset": catalog.map((record) => ({
        ...
        "url": record.url,
        "keywords": [record.category, record.coverage, ...(record.tags?.topics || []).map((value) => topicLabels[value] || value)],
