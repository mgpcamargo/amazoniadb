(() => {
  ...
  const describeFreshness = (checkedDate) => {
    ...
    else if (days < 14) relative = `há ${days} dias`;
    else if (days < 60) relative = `há ${Math.round(days / 7)} semanas`;
    else if (days < 730) relative = `há ${Math.round(days / 30)} meses`;
    else relative = `há ${Math.round(days / 365)} anos`;
    return { stale: days > STALE_DAYS, relative };
  };

  // Citation helpers. BibTeX uses "s.d." (sem data) for year rather than
  // guessing a publication year from the verification date.
  const buildApaCitation = (record) => `"${record.title}." ${record.provider}. Acessado em ${record.checked}. ${record.url}`;
  const buildBibtex = (record) => [
    ...
    `  note         = {Acessado em ${record.checked}},`,
    ...
    grid.innerHTML = records.map((record) => {
      ...
      <article class="dataset-card">
        ...
        <ul class="metadata" aria-label="Metadados do conjunto de dados">
          ...
          <li class="freshness-pill${fresh.stale ? " is-stale" : ""}"><time datetime="${escapeHtml(record.checked)}" title="Verificado em ${escapeHtml(record.checked)}">${fresh.stale ? "Revisão pendente — verificado" : "Verificado"} ${fresh.relative}</time></li>
        </ul>
        ...
        ${record.methodologyUrl ? `<p class="methodology-line"><a href="${escapeHtml(record.methodologyUrl)}" target="_blank" rel="noopener noreferrer">Documentação de metodologia <span class="sr-only">(abre em nova aba)</span></a></p>` : ""}
        <div class="card-actions">
          ...
          <div class="cite-group" role="group" aria-label="Citar este conjunto de dados">
  ...
  grid.addEventListener("click", async (event) => {
    ...
    flashConfirmation(citeButton, ok ? "Copiado" : "Não foi possível copiar", format === "bibtex" ? "BibTeX" : "Citar");
  });
